...
/*
		http
			...
			.authorizeHttpRequests((authorizeHttpRequests) -> authorizeHttpRequests
	               .antMatchers("/resources/**").permitAll()				// "/resources" 및 하위 경로에 대해서 접근을 허용 
	               .antMatchers("/", "/login", "/register").permitAll()		// "/", "/login", "/register"에 대해서 접근을 허용
...
*/
		...
		http
			...
			// 인가(Authorization)에 대해서 AcceessDeniedException이 발생한다.
			// 인증, 인가 과정에서 발생하는 예외를 처리하는 사용자정의 핸들러를 구현해서 등록할 수 있으며, 따로 설정하지 않으면 기본으로 등록되어 있는 핸들러가 동작한다.
